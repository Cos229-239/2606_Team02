...
func _build_ui() -> void:
	...
	_add_building_entry(building_list, "Ancient Tree", "Level %d" % GameState.ancient_tree_level, "Restore the grove heart and claim restoration rewards.")
	_add_building_entry(building_list, "Market Stall", "Reputation %d" % GameState.market_reputation, "Trade resources for Coins and reputation.")
	_add_building_entry(building_list, "Arcane Forge", "Level %d" % GameState.forge_level, "Forge permanent upgrades for production, potions, and pond restoration.")
